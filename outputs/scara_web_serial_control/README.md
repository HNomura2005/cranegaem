...
3x3のマスにピンポン玉を交互に置いて、まるばつゲームのように遊ぶ構成です。
...
今回のピンポン玉ゲームでは不要なので、UIとArduinoコードから外しました。
...
- `JOG Z 100`: 上下を少し動かす
- `PICK`: 玉をつかむ場所へ行って、電磁石をON
- `PLACE 1`から`PLACE 9`: 指定したマスへ行って、玉をはなす
- `MAG 1`: 電磁石ON
- `MAG 0`: 電磁石OFF
- `HOME`: おうちの位置へ戻る
- `STOP`: 停止して電磁石OFF
